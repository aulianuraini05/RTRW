<div class="space-y-6">
    @if (Auth::user()->isWarga())
        ...
        @if (! empty($schedule))
            <div class="rounded-md bg-gray-50 p-4">
                <p class="text-sm text-gray-500">Nominal Iuran {{ $schedule->jenis }} {{ $schedule->period_label }} (ditentukan Ketua RT)</p>
    ...
    @else
        <div>
            ...
            <x-text-input id="payer_name" name="payer_name" type="text" class="mt-1 block w-full" :value="old('payer_name', $contribution->payer_name ?? $contribution->user?->name ?? '')" maxlength="100" />
        ...
        <div>
            <x-input-label for="amount" value="Jumlah Pembayaran (Rp)" />
            <x-text-input id="amount" name="amount" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('amount', $contribution->amount ?? 50000)" placeholder="Contoh: 50000" required />
            <x-input-error class="mt-2" :messages="$errors->get('amount')" />
        </div>
        ...
        <div>
            ...
            <select id="payment_method" name="payment_method" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                <option value="cash" @selected(old('payment_method', $contribution->payment_method ?? 'cash') === 'cash')>💵 Tunai</option>
                <option value="qris" @selected(old('payment_method', $contribution->payment_method ?? '') === 'qris')>📲 QRIS</option>
                <option value="virtual_account" @selected(old('payment_method', $contribution->payment_method ?? '') === 'virtual_account')>🏦 Virtual Account</option>
                <option value="transfer" @selected(old('payment_method', $contribution->payment_method ?? '') === 'transfer')>💳 Transfer Bank</option>
            </select>
        ...
        <div>
            ...
            <select id="payment_status" name="payment_status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                <option value="lunas" @selected(old('payment_status', $contribution->payment_status ?? 'lunas') === 'lunas')>✅ Lunas</option>
                <option value="pending" @selected(old('payment_status', $contribution->payment_status ?? '') === 'pending')>⏳ Pending</option>
