        ...
        <div class="flex items-center justify-between gap-4">
            ...
            @else
                <div class="flex items-center gap-2">
                    <a href="{{ route('iuran_schedules.index') }}" class="rounded-md bg-white px-4 py-2 text-sm font-semibold text-indigo-600 ring-1 ring-indigo-600 hover:bg-indigo-50">Kelola Iuran Bulanan</a>
            ...
            @if (Auth::user()->isWarga() && ! empty($activeSchedule))
                <div class="rounded-lg bg-indigo-600 p-6 text-white shadow-sm">
                    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                        <div>
                            <p class="text-sm font-medium opacity-90">Iuran {{ $activeSchedule->jenis }} {{ $activeSchedule->period_label }} {{ $activeSchedule->rt?->name ? '('.$activeSchedule->rt->name.')' : '' }}</p>
                        ...
                        @if (! empty($myPendingBill))
                            <a href="{{ route('contributions.show', $myPendingBill) }}" class="rounded-md bg-white px-4 py-2 text-sm font-bold text-indigo-700 hover:bg-indigo-50">Bayar Sekarang</a>
                        ...
                        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                            <div class="flex-1">
                                ...
                                @if ($contribution->schedule)
                                    <p class="mt-0.5 text-xs font-medium text-indigo-600">Iuran {{ $contribution->schedule->jenis }} {{ $contribution->schedule->period_label }}</p>
                                @endif
                                @if ($contribution->amount)
                                    <p class="mt-1 text-sm font-semibold text-gray-700">Rp {{ number_format((float) $contribution->amount, 0, ',', '.') }}</p>
                                @endif
                                @if ($contribution->payment_code)
                                    <p class="mt-1 font-mono text-xs text-gray-500">Kode: {{ $contribution->payment_code }}</p>
                                @endif
