<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-lg text-gray-800 leading-tight">Kas {{ $kasSchedule->period_label }} — {{ $kasSchedule->rt?->name ?? '' }}</h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                <div class="rounded-lg bg-indigo-600 p-6 text-white shadow-sm">
                    ...
                    <p class="mt-2 text-xl font-extrabold">Rp {{ number_format((float) $kasSchedule->amount, 0, ',', '.') }}</p>
                    <p class="mt-1 text-xs opacity-75">Kas {{ $kasSchedule->period_label }}</p>
                </div>
                ...
                <div class="rounded-lg bg-white p-6 shadow-sm border-l-4 border-yellow-500">
                    ...
                    <form method="POST" action="{{ route('kas_schedules.sync', $kasSchedule) }}" class="mt-2">
                        @csrf
                ...
                @forelse ($transactions as $transaction)
                    <article class="rounded-lg bg-white p-5 shadow-sm">
                        <div class="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
                            <div>
                                <h3 class="text-base font-semibold text-gray-900">{{ $transaction->display_name }}</h3>
                                <p class="mt-1 text-sm text-gray-600">Rp {{ number_format((float) $transaction->amount, 0, ',', '.') }}</p>
                            </div>
                            <span class="rounded-full px-2.5 py-0.5 text-xs font-medium {{ $transaction->payment_status === 'lunas' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
            ...
            <a href="{{ route('kas_schedules.index') }}" class="inline-block text-sm font-medium text-gray-600 hover:text-gray-900">Kembali ke daftar jadwal kas</a>
        </div>
    </div>
</x-app-layout>
