        ...
        <nav class="space-y-1">
            ...
            @if(Auth::user()->isAdmin())
                <x-nav-link :href="route('warga.index')" :active="request()->routeIs('warga.*')">
                    <svg class="h-5 w-5 shrink-0" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 21v-2a4 4 0 00-4-4H9a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8zM17 11h2a2 2 0 012 2v4M21 11a2 2 0 00-2-2h-1" />
                    </svg>
                    {{ __('Data Warga') }}
                </x-nav-link>
            @endif
            ...
            <x-nav-link :href="route('notifications.index')" :active="request()->routeIs('notifications.*')">
                ...
                <span class="relative shrink-0">
                    ...
                    @if ($unreadCount > 0)
                        <span class="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-600 px-1 text-[10px] font-bold leading-none text-white">{{ $unreadCount > 99 ? '99+' : $unreadCount }}</span>
                    @endif
                </span>
                {{ __('Notifikasi') }}
            </x-nav-link>
