<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Detail aspirasi</h2></x-slot>
    ...
    <div class="py-12">
        <article class="mx-auto max-w-3xl rounded-lg bg-white p-6 shadow-sm sm:p-8">
            ...
            <h1 class="mt-3 text-2xl font-bold text-gray-900">{{ $aspiration->aspiration_title }}</h1>
            @if (Auth::user()->isAdmin())
                <p class="mt-2 text-sm text-gray-500">Diajukan oleh: {{ $aspiration->user?->name ?? 'Data warga lama' }}</p>
            @endif
            <div class="mt-6 whitespace-pre-line leading-7 text-gray-700">{{ $aspiration->aspiration_content }}</div>
            @if (Auth::user()->isAdmin())
                <div class="mt-8 border-t pt-6">
                    @include('aspirations._status-actions', ['aspiration' => $aspiration])
                </div>
            @endif
            <a href="{{ route('aspirations.index') }}" class="mt-8 inline-block border-t pt-6 text-sm font-medium text-gray-600 hover:text-gray-900">← Kembali ke daftar</a>
        </article>
    </div>
</x-app-layout>
