<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <h2 class="font-semibold text-lg text-gray-800 leading-tight">Iuran Bulanan per RT</h2>
            <div class="flex items-center gap-2">
                <a href="{{ route('contributions.index') }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">Batal</a>
                <a href="{{ route('iuran_schedules.create') }}" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">+ Tentukan Iuran Bulan Ini</a>
            </div>
        </div>
    </x-slot>
        ...
        <div class="space-y-6">
            ...
            @if (! Auth::user()->isRt())
                <div class="rounded-lg bg-white p-4 shadow-sm">
                    <form method="GET" action="{{ route('iuran_schedules.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="space-y-3">
                        ...
                        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                            <div class="flex-1">
                                <div class="mb-1 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                                    ...
                                    <span>•</span>
                                    <span>{{ $schedule->paid_count }} / {{ $schedule->contributions_count }} warga sudah bayar</span>
                                </div>
                                <h3 class="text-base font-semibold text-gray-900">
                                    <a href="{{ route('iuran_schedules.show', $schedule) }}" class="hover:text-indigo-600">
                                        Iuran {{ $schedule->jenis }} {{ $schedule->period_label }}
                                    </a>
                                </h3>
                            ...
                            <div class="flex flex-wrap items-center gap-2 border-t pt-3 sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0">
                                <a href="{{ route('iuran_schedules.show', $schedule) }}" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800">Lihat Tagihan</a>
                                <form method="POST" action="{{ route('iuran_schedules.destroy', $schedule) }}" onsubmit="return confirm('Hapus jadwal iuran ini? Tagihan yang belum dibayar ikut terhapus.')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <div class="rounded-lg bg-white p-8 text-center text-gray-600 shadow-sm">
                        Belum ada jadwal iuran bulanan.
                    </div>
                @endforelse
